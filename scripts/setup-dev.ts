...
import { ensureHookdeckConnections, updateWebhookSource } from '../src/lib/hookdeck/initialize';
...
async function main() {
  console.log('🚀 Deep Queue Development Setup\n');
  console.log('This script will help you set up your environment for local development.\n');
  console.log('Requirements:');
  console.log('- A Hookdeck account (https://hookdeck.com)');
  console.log('- An OpenAI API account (https://platform.openai.com)');
  console.log('- Demo authentication uses simple username/password (default: demo/password)\n');
  ...
  const customAuth = await question('Use custom demo credentials? (y/N - default: demo/password): ');

  let demoUsername = 'demo';
  let demoPassword = 'password';
  if (customAuth.toLowerCase() === 'y') {
    demoUsername = await question('Enter demo username (default: demo): ') || 'demo';
    demoPassword = await question('Enter demo password (default: password): ') || 'password';
  ...
  console.log('You need a Hookdeck account to handle webhook queuing and routing.');
  ...
  const hookdeckSigningSecret = await question('Enter your Hookdeck signing secret: ');
  if (!hookdeckSigningSecret.trim()) {
    console.error('❌ Hookdeck signing secret is required for webhook verification');
    process.exit(1);
  }
  
  envContent = replaceEnvVar(envContent, 'HOOKDECK_API_KEY', hookdeckApiKey);
  envContent = replaceEnvVar(envContent, 'HOOKDECK_SIGNING_SECRET', hookdeckSigningSecret);
  ...
  console.log('You need an OpenAI API key with access to the Deep Research API.');
  ...
  const connections = await ensureHookdeckConnections(hookdeckApiKey, openaiApiKey, appUrl);
  ...
  console.log('   - Go to https://platform.openai.com/settings/project/webhooks');
  ...
  await updateWebhookSource(connections.webhook.sourceId, openaiWebhookSecret.trim(), hookdeckApiKey);
  ...
  console.log(`4. Login with demo credentials (username: ${demoUsername}, password: ${demoPassword})`);
