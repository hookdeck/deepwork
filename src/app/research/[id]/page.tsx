...
export default function ResearchDetailPage() {
  ...
  const { data: research, error: researchError, isLoading: researchLoading } = useSWR<Research>(
  ...
  const { data: eventsData, error: eventsError, isLoading: eventsLoading } = useSWR<{ events: EventTimeline[] }>(
    ...
    return (
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6 text-center">
          <p className="text-red-600 dark:text-red-400">Failed to load research details. Please try again.</p>
          <button
            onClick={() => router.push('/')}
  ...
  const getStatusBadge = (status: Research['status']) => {
    ...
    return (
      <span className={`px-3 py-1.5 text-sm font-medium rounded-full ${statusStyles[status] || statusStyles.pending}`}>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Research Details</h1>
      </div>
            ...
            {researchLoading ? (
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-gray-900 dark:text-gray-100 text-lg">{research.question}</p>
                </div>
              ...
              <p className="text-center text-gray-500 dark:text-gray-400">Research not found</p>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="mb-8">
        <div className="bg-white dark:bg-gray-800 shadow rounded-lg border border-gray-200 dark:border-gray-700">
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Hookdeck Events Timeline</h2>
          </div>
      ...
      {research?.status === 'completed' && research.result && (
        <section>
          <div className="bg-white dark:bg-gray-800 shadow rounded-lg border border-gray-200 dark:border-gray-700 p-6 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              Download the complete research results and event timeline as a JSON file.
            </p>
            <button
