...
async function createHookdeckConnection(
  ...
  log(
    "debug",
    `Creating/updating Hookdeck connection: ${connectionName}`,
    config
  );
...
export async function ensureHookdeckConnections(
  ...
  try {
    ...
    const existingConnections = await kvStore.get<StoredConnections>(
    ...
    if (error instanceof Error) {
      if (
        ...
        throw new Error(
          "Invalid Hookdeck API key. Please check your HOOKDECK_API_KEY environment variable."
        );
      }
      if (error.message.includes("404")) {
        throw new Error(
          "Hookdeck API endpoint not found. Please check your API version."
        );
      }
    }
    throw error;
